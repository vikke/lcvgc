...
- 迷いが発生した場合は、こちらに聞く事。その際、3～5パターンの案を提示し、推奨するパターンを提示する事。
    - 推奨するパターンは `修正量`、 `難しさ` 、ではなく、実装の柔軟性や分離性、筋の良さで選ぶ事。
    - 他の選ばれなかったバターンは選ばれなかった理由と強みを出力し、こちらの選択の為の情報を出す事。
...
- 調査系以外の編集・新規系のpromptはPRを作成する。
- **厳守**: 以下の手順をブランチ作成からPR作成まで一気通貫で実行する事。
  1. ブランチ作成
  2. empty commit 作成
  3. `gh pr create` でPR作成（タイトル・ボディはpromptから生成, asignees には `@me` を指定すること）
  4. PRのURL表示
  5. 実装開始
- **アンチパターン（絶対禁止）**:
  - ブランチだけ作って調査を始める
