...
* [10. 再生制御](#10-再生制御)
    ...
    * [10.3 停止](#103-停止)
* [11. マイク入力によるメロディー入力 (lcvgc-mic)](#11-マイク入力によるメロディー入力-lcvgc-mic)
    * [11.1 アーキテクチャ](#111-アーキテクチャ)
    * [11.2 コマンドラインオプション](#112-コマンドラインオプション)
    * [11.3 出力フォーマット](#113-出力フォーマット)
    * [11.4 Neovim連携](#114-neovim連携)
    * [11.5 Cargo依存（lcvgc-mic）](#115-cargo依存lcvgc-mic)
    * [11.6 実装優先度](#116-実装優先度)
* [12. エラーハンドリング](#12-エラーハンドリング)
    * [12.1 eval失敗](#121-eval失敗)
    * [12.2 未定義の参照](#122-未定義の参照)
    * [12.3 削除操作](#123-削除操作)
    * [12.4 エンジン内部のパニック](#124-エンジン内部のパニック)
    * [12.5 MIDI出力エラー](#125-midi出力エラー)
    * [12.6 Neovim接続断](#126-neovim接続断)
* [13. 文法ルールまとめ](#13-文法ルールまとめ)
...
## 11. マイク入力によるメロディー入力 (lcvgc-mic)

鼻歌やマイク入力からピッチを検出し、DSLの音名テキストとしてNeovimバッファに直接挿入する。エンジン本体とは別バイナリ (`lcvgc-mic`) として実装する。

### 11.1 アーキテクチャ

```
マイク → lcvgc-mic (ピッチ検出) → stdout (音名テキスト) → Neovimバッファに挿入
```

lcvgc-micはエンジンに依存せず、単体で動作する。出力はlcvgcのDSL音名フォーマットに準拠する。
...
Options:
  --quantize <N>       音符の最小単位 (4, 8, 16)。デフォルト: 8
  --threshold <0-1>    音量閾値。これ以下は休符とみなす。デフォルト: 0.3
  --device <name>      オーディオ入力デバイス名。省略でシステムデフォルト
  --key <note>         基準キー。スケール外の音を最寄りのスケール音に補正。省略で補正なし
  --scale <type>       --key と組み合わせて使用。デフォルト: minor
```

### 11.3 出力フォーマット

stdoutにlcvgcのDSL音名を出力する。省略記法に準拠し、オクターブ・音長は変化時のみ出力。
...
- 無音区間は `r` を出力
- 音長はquantize単位に丸められる
- `--key` / `--scale` 指定時、検出されたピッチをスケール内の最寄りの音に補正する

### 11.4 Neovim連携

Neovimプラグイン側で `lcvgc-mic` をジョブとして起動し、出力をカーソル位置に挿入する。

カーソル位置のclipに `[scale ...]` が指定されている場合、`--key` と `--scale` を自動で付与する。明示的に指定すればオーバーライド可能。

```
clip melody_a [bars 4] [scale c minor] {
  lead |                     ← カーソルをここに置いて
  :LcvgcMicStart             ← --key c --scale minor が自動付与
  (ふんふんふん♪)             ← Cマイナースケールに自動補正される
  :LcvgcMicStop
  lead c:3:8 d eb f::4 g::2  ← スケール内の音名のみ入力される
}

// scaleなしのclipではスケール補正なし（検出されたピッチそのまま）
...
### 11.5 Cargo依存（lcvgc-mic）

```toml
[dependencies]
cpal = "0.15"               # オーディオ入力
pitch-detection = "0.3"     # ピッチ検出（McLeodアルゴリズム）
clap = { version = "4", features = ["derive"] }
```

### 11.6 実装優先度

Phase 4以降。エンジン本体とNeovimプラグインの完成後に着手する。
