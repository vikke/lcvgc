...
> - `stop <clip>`（現行は mute 相当）は将来 `mute <clip>` にリネーム予定（Issue #43）
> - `pause` / `resume` は **実装済み**（§10.4.1 の表の pause/resume 行すべて、Issue #44）
> - `mute <clip>` / `unmute <clip>` は未実装（#43 で `stop <clip>` をリネーム予定）
>
> 以下の表は目標仕様。`stop <clip>` を使っているコードは #43 完了後に `mute <clip>` に書き換える必要がある。
...
| 操作 | tick | 音 | 用途 |
|---|---|---|---|
| `stop <clip>` | — | — | **存在しない**（`stop` は scene/session/全体専用） |
...
- 各ブロック（device, instrument, kit, clip, scene, session, tempo, play, stop, pause, resume, include, var）は独立してパース・eval可能
