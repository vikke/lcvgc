...
* [7. クリップ定義 (clip)](#7-クリップ定義-clip)
    ...
    * [7.4 オクターブシフト (`[>>]` / `[<<]`)](#74-オクターブシフト---)
    * [7.5 音程楽器の記法](#75-音程楽器の記法)
        * [パースルール](#パースルール)
    * [7.6 省略記法](#76-省略記法)
        * [単音の省略](#単音の省略)
        * [コード名の省略](#コード名の省略)
        ...
        * [和音内の省略](#和音内の省略)
    * [7.7 繰り返し](#77-繰り返し)
    * [7.8 アーティキュレーション（Gate制御）](#78-アーティキュレーションgate制御)
        * [通常（ノーマル）](#通常ノーマル)
        * [スタッカート `'`](#スタッカート-)
    ...
    * [7.9 複数行記述](#79-複数行記述)
    * [7.10 小節ジャンプ (`>N`)](#710-小節ジャンプ-n)
    * [7.11 `|` 拍境界スナップ（音程楽器）](#711--拍境界スナップ音程楽器)
    * [7.12 和音（角括弧記法）](#712-和音角括弧記法)
    * [7.13 コード名記法](#713-コード名記法)
    * [7.14 アルペジオ](#714-アルペジオ)
    * [7.15 ドラム（ステップシーケンサー記法）](#715-ドラムステップシーケンサー記法)
        * [ヒット記号](#ヒット記号)
        * [`|` 拍境界スナップ](#-拍境界スナップ)
        * [`>N` 小節ジャンプ](#n-小節ジャンプ)
        * [繰り返し](#繰り返し)
        * [確率行](#確率行)
    * [7.16 CCオートメーション](#716-ccオートメーション)
        * [ステップ方式](#ステップ方式)
        * [時間指定方式](#時間指定方式)
        * [指数カーブ補間](#指数カーブ補間)
        * [両方式の混在](#両方式の混在)
...
scene 内 `tempo` 行は **scene が 1 ループ完了するたびに** 評価され clock に適用される。`tempo +5` は累積、`tempo 120` は毎ループ 120 に再セット（冪等）。scene を `play` した直後 1 ループ目には適用されず、最初のループ完了境界で初めて反映される。session で別 scene へ遷移する境界も「直前 scene のループ完了」として直前 scene の tempo 行が適用される。

`tempo +N` / `tempo -N` の符号と数値の間には**行内空白（スペース・タブ）を任意個数挟める**（改行は不可）。`tempo +5`, `tempo + 5`, `tempo +  \t  5` はすべて同義。
...
この**4小節グリッド**は、楽譜上の小節位置ではなく **transport起点（曲頭 = play 開始）から実際に演奏した小節数**で数える（累積 tick `transport_tick` を `4 × 1小節tick` で割った境界）。scene の遷移や session の `[loop]` エントリの繰り返しでリセットされず、曲頭から一貫して4小節ごとに刻まれる。session 再生中に scene が切り替わっても同じグリッド上に乗る。
...
clip 定義のオプション位置に `[>>]` / `[<<]` を書くと、その clip 全体のピッチを 1 オクターブ（12 半音）単位で上下にシフトできる。

```
// clip 全体を +1 オクターブ
clip lead_up [>>] {
  lead c:4:4 e g    // 実際の発音は C5, E5, G5
}

// clip 全体を -1 オクターブ
clip bass_down [<<] {
  bass c:3:4 e g    // 実際の発音は C2, E2, G2
}

// [>>] / [<<] を複数並べると合算される
clip lead_up2 [>> >>] {  // +2 オクターブ
  lead c:4:4
}

clip net_up [>> >> <<] {  // +1 オクターブ（合算結果）
  lead c:4:4
}
```
...
- `[>>]` … +1 オクターブ、`[<<]` … -1 オクターブ。
- 括弧内に `>>` / `<<` を空白区切りで複数並べると **合算** される（`[>> >>]` で +2、`[>> <<]` で ±0）。
- `[>>] [>>]` のように括弧を分けて複数指定した場合も合算される。
- `[bars ...]` / `[time ...]` / `[scale ...]` と任意の順序で併記できる。
- シフトの対象は **音程楽器（pitched）の clip のみ**。ドラム clip では効果を持たない。
- シフト後の MIDI ノート番号が範囲 (0-127) を外れる場合は **コンパイルエラー** となる。
- シフトは clip 内の明示オクターブ指定（`c:3` など）にも省略時の引き継ぎオクターブにも一律で効く。
...
- コード名: `cm7`, `fM7`, `g7` 等（サフィックス一覧はセクション7.12参照）
...
- `(`/`)`/`*`/数字の境界には **空白 (スペース・タブ・改行) を任意個数挟める**。以下はすべて同義:
...
- `*N` を **省略した `( ... )` は単なるグルーピング括弧** として扱われ、中身がそのまま展開される。整形のために括弧でくくる用途や、`*N` の有無を後から切り替える編集に使える。

```
lead ( c d e f )    // ≡  lead c d e f
```

### 7.8 アーティキュレーション（Gate制御）
...
ノートの末尾に `vN` を付けると、そのノートの Note On に対する MIDI velocity を上書きする。

- `N` の範囲は `0` – `127` (MIDI velocity 値域)。
- `vN` を省略した場合、コンパイラは Note On の velocity を `100` で出力する。
- `vN` は Note On のみに作用し、対応する Note Off の velocity は常に `0`。
- `vN` は gate 修飾子 (`'` / `gN`) や付点 (`.`) と独立で、これらと自由に組合せできる。

```
clip bass_velocity [bars 1] {
  bass c:3:4v127 d:4 e:4v40 f:4         // accent、通常、ghost、通常
  bass c:3:4.v90 d:4'v110 e:4g95v100    // 他の修飾子と組合せ可能
}
```

#### サフィックス修飾子の順序

- **付点 `.`** は音価の一部として扱い、音価フィールドの直後に置く (`c:3:4.`)。他の修飾子と順不同にはできない。
- アーティキュレーション / velocity 系修飾子 — `'`（スタッカート）/ `gN`（Gate比率直接）/ `vN`（Velocity 直接）— には以下が適用される。
  - **順序は不同**: `c:3:4g95v100` と `c:3:4v100g95` は同じノートとして解釈される。
  - **各修飾子は高々 1 回**まで指定可能。重複指定 (例: `c:3:4v100v90`、`c:3:4g30g50`) はパースエラー。
  - **`'` と `gN` は両立不可**。どちらも gate 比率を決めるため、両方指定 (例: `c:3:4'g95`) はパースエラー。
- ノート本体と修飾子の間、および修飾子同士の間に空白を入れることはできない。

#### 記法まとめ

| 記法 | 意味 | 例 |
|------|------|------|
| `c:3:4` | 通常（gate_normal適用、velocity 100） | Gate On = 音長×80%、Velocity = 100 |
...
| `c:3:4v127` | Velocity 直接指定（127） | Velocity = 127 |
| `c:3:4.g30v90` | 付点 + Gate 30% + Velocity 90 (g/v は順不同) | Gate On = 付点音長×30%、Velocity = 90 |
| `c:3:4.v90g30` | 同上 (g/v 入替) | Gate On = 付点音長×30%、Velocity = 90 |
...
clip内で同じ楽器名の行が続くと、前の行の続きとして**時系列に連結**される。長いclipを読みやすく分割できる。オクターブ・音長の引き継ぎ (carry-over) は行をまたいで維持される。1行に何小節分書くかは自由。
...
- 行と行の間にコメント行や空行があっても、連続性は維持される (= 連結)
- 行をまたいで `octave` / `duration` / 付点 (`.`) が引き継がれる
- 連結されるのは **同じ楽器名の連続行のみ**。 別の楽器名が現れた時点で、その行は新しい**並列レイヤー** (`current_tick = 0` から開始、 carry-over もリセット) として扱われる

#### 並列レイヤーと `---` セパレータ

複数の行を「**並列に重ねたい**」ときは、別の楽器名を使うか、 `---` (3 文字のハイフンの独立行) を挟む。 `---` の前後は同じ楽器名でも別の並列レイヤーとして扱われる (どちらも `tick 0` から発音、 carry-over はリセット)。

```
clip layered [bars 1] {
  // 同じ chord 楽器を 2 レイヤーに重ねる例
  chord [c:4 e g]:1
  ---
  chord [g:5 b:5 d:6]:1
}
...
- ハイフンは厳密に**3文字**。 `--` や `----` はセパレータと見なされない
- 前後の水平方向空白 (スペース、タブ) は許容
- 行末は改行 / EOF / `}` (clip body の終端)
- ドラム clip body 内でも `---` を書けるが、現在は no-op として消費される (将来のレイヤー機能に予約)

ドラムも同様に同じ楽器名の行が連結される。確率行は直前のヒット行にだけ対応する。
...
音程楽器 / ドラム / CC ステップ方式のいずれでも使用できる。`|`（拍境界スナップ）とは別の記号なので混乱しない。
...
音程楽器 (chord / lead / bass 等) の clip 行でも `|` で拍境界に揃えられる。`|` の位置までの累積長 (tick) を見て、次の挙動を行う:

- 累積長が 1 拍未満 → **不足分を休符で埋めて** 次拍境界まで進める
- 累積長が 1 拍を超えている → **直前の拍境界まで切り戻す** (既に書き出した末尾の音を超過分だけ削る)

ドラム / CC のセル単位スナップ (`beats_per_step` 換算) と内部実装は異なるが、ユーザから見える意味は同じである。

```
// 不足: 8分音符 × 1 = 半拍。| が次拍境界まで休符で埋める。
//      → 続く d:3 は 2 拍目頭から鳴る
bass c:3:8 | d:3:4

// 超過: 8分 × 5 = 1 拍半。| で直前拍境界まで戻し、5 個目の c を削る
//      → 実質 c c c c の 4 音 (1 拍ぶん) として鳴り、d:3 は 2 拍目頭から
bass c:3:8 c c c c c | d:3:4
```

`|` は 1 拍 (`time` で指定した拍長) 単位でのみ整列する。複数行記述 (7.8) と組み合わせれば、行末にちょうど拍境界が来るよう揃えるのにも使える。
...
- 音符解像度: `4`, `8`, `16` 等（1音あたりの発音長）。省略可能。

#### 1音あたりの長さの決定ルール

| `arp` の第2引数 | 和音 `[..]:D` | 1音あたりの長さ |
|---|---|---|
| 指定あり | 指定あり | **`arp` の第2引数を優先**（和音側の `:D` は無視） |
| 指定あり | 指定なし | `arp` の第2引数 |
| 指定なし | 指定あり | 和音側の `:D` |
| 指定なし | 指定なし | LSP エラー（音価が決定できない） |

- `random` 方向はループ毎に新しい順序が抽選される。
- `updown` は両端を二度鳴らさない ping-pong（例: `[c e g]` → `c, e, g, e`）。

### 7.15 ドラム（ステップシーケンサー記法）
...
`|` は現在位置を拍境界 (resolution 16 なら 4 セル境界、resolution 8 なら 2 セル境界) に揃える。直近の `|`/行頭から `|` 直前までのセル数を `beats_per_step` で換算し、次のように処理する:

- セル数 ≤ 1 拍分 → **不足分を `.` (休符) で埋めて** 次拍境界まで進める
- セル数 > 1 拍分 → **直前の拍境界まで切り落とす** (超過セルを捨てる)
...
超過のケースでは、`|` 直前のセルを拍境界まで切り戻す:

```
bd    x.x.x.x.x. |
// 5 セル (1 拍 + 1 セル) → 直前拍境界まで切り戻して 4 セルに
// 展開: x.x.x.x.

bd    xxxxx |
// 同じく超過 1 セルぶん切り戻して 4 セルに
// 展開: xxxx
```

#### `>N` 小節ジャンプ

セクション 7.9 の `>N` をドラム行でも使える。スペースの有無は任意。
...
`|` (拍境界スナップ) と `>N` (小節ジャンプ) は別記号で、両者を同じ行に混在させても問題ない。
...
セクション 7.6 と同じく、`(`/`)`/`*`/数字の境界に空白 (スペース・タブ・改行) を任意個数挟める。`*N` を省略した `(...)` は単なるグルーピングとして中身がそのまま展開される (`(x.x.)` ≡ `x.x.`)。
...
- 確率行を省略すれば全て100%
- 判定はループのたびに毎回行う
- `|` 拍境界スナップが使える。不足は `.`（100%）で埋め、超過は直前拍境界まで切り落とす。ヒット行と同じ展開ルール
- `>N` 小節ジャンプが使える。ヒット行と同じ展開ルール
...
ステップ方式ではドラムと同じセル列メタトークンが使える:

| 記号 | 意味 |
|------|------|
| `0`-`127` | その step で CC 値を送出する |
| `.` | **その step では CC を送出しない**（ホスト側の値を保持） |
| `\|` | 拍境界スナップ。不足は `.` (= 送出しない) で埋め、超過は直前拍境界まで切り落とす |
| `>N` | N 小節目の頭にジャンプ。drum と同じセマンティクス |
| `(...)*N` | グルーピングと繰り返し |

`.` を使うと「ノートの位置でだけ CC を動かす」表現が直感的に書ける。
...
clip bass_sync [bars 1] {
  ...
  // 音の発音と同期して cutoff を変えたい (= ノートと同じ位置にだけ値を置く)
  bass.cutoff 30 . . 60 . . . . 90 . . . . . . .
  //          ^^^^^^^ 1 拍目で 30、4 ステップ目で 60、9 ステップ目で 90 に変化
  //          途中の `.` ではメッセージを送らないため、ホスト側の値が保持される
}
```

繰り返しや小節ジャンプも組み合わせられる:
...
評価タイミングと構文の詳細は §4 参照。要点:
- 適用は **scene のループ完了境界** で 1 回。1 ループの最中に bpm は変わらない。
- `+N` / `-N` は累積、`N` （絶対値）は毎ループ N に再セット（冪等）。
- 符号と数値の間には行内空白を任意個数挟める（`tempo + 5`, `tempo +  \t  5` 等）。改行は不可。
...
#### 更新起因の4小節グリッド前倒し（§7 と同じグリッド）

session 再生中に、その session が参照している要素 — 使用中 clip / 再生中 scene の構成 / session 定義そのもの — が eval で上書きされた場合、現在再生中の scene を**そのループ（bars）を再生しきるまで待たず**、§7 と同じ **4小節グリッド（曲頭から演奏した小節数）の直近の区切り**で次のエントリ（別 scene）へ強制的に切り替える。長い bars の scene でも切替までの待ちは最大4小節に収まる。

- **トリガー**: ① 再生中 scene が使う clip の上書き、② 再生中 scene の構成（`scene` 定義）の上書き、③ 再生中 session 定義の上書き、のいずれか。更新が無ければ通常どおり各エントリの `[repeat N]` / `[loop]` を最後まで消費する（前倒しは起きない）。
- **repeat の扱い**: 前倒し遷移では現在エントリの残り `[repeat]` を破棄して次エントリへ進む。`[loop]` エントリ（通常は次に進まない）も、更新起因のときは次エントリへ強制的に進む。
- **session 定義上書き時**: pending として積まれた新 session 定義は、従来どおり**エントリ境界**で新 runner へ差し替わる。前倒し遷移はそのエントリ境界を4小節グリッドへ早めるだけで、差し替えタイミングのセマンティクス自体は変えない。
- clip 単位の差し替え（§7）と session の scene 前倒し遷移は、いずれも同じ `transport_tick`（曲頭から演奏した tick）の4小節グリッドで判定され、互いに揃う。

session内のシーンに `[loop]` を付けると、そのシーンで無限ループし次に進まない。次に進ませるには新しいplayをevalする（または上記の更新起因の前倒し遷移）。
...
- clipオプション `[>>]` / `[<<]` でclip全体を1オクターブ単位で上下シフトできる（合算可・ピッチド clip 専用・範囲外はコンパイルエラー）
...
- 音程楽器ノートの velocity 既定値は 100。ノートに `vN`（N = 0–127）を付けると Note On の velocity を上書きする。アーティキュレーション / velocity 系修飾子 (`'` / `gN` / `vN`) は順序不同で、各高々 1 回、`'` と `gN` は両立不可。付点 `.` は音価フィールド直後に置く既存仕様を維持する
