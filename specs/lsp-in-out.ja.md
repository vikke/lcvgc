...
```json
{"type": "lsp_completion", "source": "<DSL ソーステキスト>", "offset": <バイトオフセット>}
```
...
```json
{"type": "lsp_hover", "source": "<DSL ソーステキスト>", "offset": <バイトオフセット>}
```
...
```json
{"type": "lsp_diagnostics", "source": "<DSL ソーステキスト>", "file_path": "/path/to/file.cvg"}
```
...
| `source`    | string          | DSL ソーステキスト全文                                       |
| `file_path` | string \| null  | ファイルパス（省略可）。指定時はinclude先の定義も解決する     |
...
```json
{"type": "lsp_goto_definition", "source": "<DSL ソーステキスト>", "offset": <バイトオフセット>}
```
...
```json
{"type": "lsp_document_symbols", "source": "<DSL ソーステキスト>"}
```
