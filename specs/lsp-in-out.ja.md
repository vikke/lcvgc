...
> **例外（サーバー起点プッシュ）**: `subscribe_midi_in`（4.10）で MIDI 入力を
> 購読した接続に限り、上記の 1 リクエスト = 1 レスポンスの原則を超えて、
> デーモンがリクエストを伴わずに `midi_in_event` 電文（第 4.12 節）を能動的に
> 送出する。この購読状態のみ、当該接続のスコープに紐づくセッション状態として
> 保持される（接続切断、または `unsubscribe_midi_in` で解除）。プッシュ電文は
> 通常のレスポンスと同じ改行区切り JSON として同一接続に混在して届くため、
> クライアントは最上位の `type` フィールドで両者を判別する。
...
### 4.10 subscribe_midi_in（MIDI入力購読開始）

指定した MIDI 入力ポートを購読し、以降この接続上で、演奏されたノートを
DSL トークン化した `midi_in_event` 電文（第 4.12 節）を非同期に受け取る。
即時レスポンスとして購読の成否を返す。

1 接続につき購読は 1 つ。購読中に再度 `subscribe_midi_in` を送ると、購読先を
新しいポートへ張り替える。購読は接続が切れるか `unsubscribe_midi_in`（4.11）を
送るまで継続する。
...
| フィールド | 型     | 説明                                                       |
|-----------|--------|------------------------------------------------------------|
| `type`    | string | 固定値 `"subscribe_midi_in"`                               |
| `port`    | string | 購読する入力ポート名（`list_ports` の `direction:"in"` のいずれか） |
...
ポートが見つからない、または MIDI サブシステムに接続できない場合はエラーを
返す。エラー時も接続は切断されず、後続のリクエストは通常どおり処理される。

---

### 4.11 unsubscribe_midi_in（MIDI入力購読解除）

現在の接続の MIDI 入力購読を解除する。購読していない場合も成功を返す（冪等）。
...
| フィールド | 型     | 説明                              |
|-----------|--------|-----------------------------------|
| `type`    | string | 固定値 `"unsubscribe_midi_in"`    |
...
### 4.12 midi_in_event（サーバー起点プッシュ電文）

`subscribe_midi_in`（4.10）で購読中の接続に対し、デーモンがリクエストを
伴わずに能動的に送出するイベント電文。MIDI 入力ポートで受信した**発音
（NoteOn かつ velocity > 0）**を 1 件ずつ DSL の音程トークンに変換して通知する。

> **注意**: 本電文はレスポンスではなくサーバー起点のプッシュである。通常の
> レスポンス（`success` フィールドを持つ）とは異なり、最上位に `type` フィールドを
> 持つ。クライアントは受信行の `type` の有無で両者を判別する。
>
> **対象外**: NoteOff、velocity 0 の NoteOn、Control Change、Program Change、
> System Real-Time（Clock/Start/Stop/Continue）は DSL テキストを生成しないため
> 送出されない。SysEx・タイミングクロック・アクティブセンシングはデーモン側で
> 無視される。
...
| フィールド | 型            | 説明                                                  |
|-----------|---------------|-------------------------------------------------------|
| `type`    | string        | 固定値 `"midi_in_event"`                              |
| `dsl`     | string        | DSL 音程トークン（`name:octave` 形式、例 `c:4` / `c#:4`）。clip 本体にそのまま挿入できる自己完結表記 |
| `note`    | number        | MIDI ノート番号 (0-127)                                |
| `raw`     | array(number) | 受信した生 MIDI バイト列（デバッグ・拡張用）            |

> **オクターブ規約**: `note` 60 = `c:4`（MIDI 60 = C4）。音名は小文字 + `#`
> 表記で、第 4.x 節の音程トークン文法（`楽器名 音名[:オクターブ][:音長]`）に
> 準拠する。音長・タイミングは付与されない（リアルタイム 1 イベント単位の変換）。
